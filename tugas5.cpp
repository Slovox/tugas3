		...
		int hitung()
		{
			...
			cout << "Masukkan Nominal: ";
    			...
    			while(!(cin >> x) || (x > 10000000) || (x < 0))
    			{
        			...
        			cout << "Masukkan Nominal dalam bentuk angka.  Try again: ";
    				    			
    			}
    		if((saldo - x) < MIN_SALDO)
    		{
    			cout << "Penarikan Gagal. Saldo anda melewati batas minimum saldo tersimpan" << endl;
    			cout << "Batas Minimum saldo tersimpan adalah " << MIN_SALDO << endl;
...
class Deposit: public Uang
{
	public:
 		...
 		int hitung()
		{
    		...
    		if(x > 10000000)
    		{
    			cout << "Deposit Gagal. Saldo anda melewati batas maximum deposit dalam satu hari" << endl;
    			cout << "Batas Maximum deposit adalah " << MAX_SALDO << endl;
    			cout << "Saldo Anda Sekarang: " << saldo;
    		}
			else
			{
...
{
	private:
		string nama  = "Budi", kode = "Pembayaran Toko*edia";
		string x, z;
		int y;
		int biaya = 50000;
...
void display_sp(int n)
{
        ...
        if (!inFile)
        {
                cout << "File tidak dapat dibuka, tekan Enter...";
                return;
        }
        cout << "\nDetail Pembelian\n";
        ...
        inFile.close();
        if (flag == false)
                cout << "\n\nNomor pembelian tidak terdaftar...";
}
...
void bayar()
{
	...
	Pembayaran pbr;
	cout << "\n\t\t\tPembelian Tiket Kereta" << endl;
	accountnumber = rand();
	cout << "\nNomor Pembayaran anda : " << accountnumber;
